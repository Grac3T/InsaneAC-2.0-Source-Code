...
extends PacketCheck {
    ...
    public KillAuraA() {
        ...
        this.setMaxViolation(InsaneAC.getInstance().getAlerts().getInt("CheckTypes.KillAura A.Alerts"));
        this.violations = -5.0;
    }
    ...
    public void handle(Player player, PlayerData playerData, Packet packet, long timestamp) {
        if (packet instanceof PacketPlayInFlying) {
            ++this.ticks;
        } else if (packet instanceof PacketPlayInUseEntity && ((PacketPlayInUseEntity)packet).a() == PacketPlayInUseEntity.EnumEntityUseAction.ATTACK && !playerData.hasLag()) {
            if (this.ticks <= 8) {
